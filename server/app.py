...
def bakeries():
    ...
    if not bakeries:
        response = make_response(jsonify({'message': 'No bakeries available'}), 404)
    else:
...
def most_expensive_baked_good():
    ...
    if not goods_by_price:
        response = make_response(jsonify({'message': 'No baked goods available'}), 404)
    else:
        goods = []
